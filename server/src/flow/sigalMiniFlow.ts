...
export function createSigalMiniFlowGraph(): FlowGraph {
  ...
  const speedFiber = b.stage(
    "speed_fiber",
    "מהירות סיבים",
    "אילו מהירות תרצה? שלוש מאות מגה, שש מאות מגה, או גיגה?",
    { x, y },
  );
  ...
  const price = b.stage(
    "price",
    "מחיר נוכחי",
    "כמה את משלמת היום על החבילה שלך?",
    { x, y },
  );
  y += dy;
  const offer = b.stage(
  ...
  const addons = b.stage(
    "addons",
    "תוספות",
    "האם תרצה להוסיף שירותים כמו VOD, ערוצי ספורט, או פרטים נוספים?",
    { x, y },
  );
  y += dy;
  ...
  const callback = b.stage(
    "callback",
    "שיחה חוזרת",
    "תרצי שאחד הנציגים שלנו יחזור אלייך לתיאום התקנה?",
    { x, y },
  );
