...
- **THEN** Sigal speaks the configured opening (with opt-out) before listening
...
Operators SHALL correct AI transcript lines on the Calls screen. Corrections SHALL be stored as **approved examples** (customer context, optional bad AI text, corrected text) and retrieved for similar future customer utterances in agent mode.
